...
class DialogoTurneroAgregarLink extends React.Component {
  constructor(props) {
    super(props);
  ...
  guardar = data => {
    ...
    if (StringUtils.isEmpty(link)) {
      this.setState({
        errorVisible: true,
        errorMensaje: "Ingrese el enlace"
      });
      return;
    }

    this.setState({ errorVisible: false, cargando: true }, () => {
      Rules_Turnero.agregarLink({
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <DialogoForm
          titulo="Agregar link en Turnero"
          inputs={inputs}
...
let componente = DialogoTurneroAgregarLink;
